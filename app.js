...
const MAX_DIM = 1100;       // حداکثر ابعاد کاری برای ادیت (سرعت بیشتر روی موبایل)
...
function applyPixelAdjustments(srcCanvas, adj) {
  ...
  const imgData = srcCanvas.getContext('2d', { willReadFrequently: true }).getImageData(0, 0, w, h);
...
function restoreSnapshot(snap, cb) {
  ...
  const done = () => {
    ...
    c1.getContext('2d', { willReadFrequently: true }).drawImage(img1, 0, 0);
    const c2 = makeOpaqueCanvas(img2.width, img2.height);
    c2.getContext('2d', { willReadFrequently: true }).drawImage(img2, 0, 0);
    S.subjectCanvas = c1;
...
function applyCropPad(paddingPct) {
  ...
  newSubject.getContext('2d', { willReadFrequently: true }).drawImage(
    S.subjectCanvas, box.x, box.y, box.w, box.h, padX, padY, box.w, box.h
  );

  const newBase = makeOpaqueTransparent(newW, newH);
  newBase.getContext('2d', { willReadFrequently: true }).drawImage(
...
function openCropTool(body) {
  ...
  $('cropApply').onclick = () => {
    ...
    newSubject.getContext('2d', { willReadFrequently: true }).drawImage(S.subjectCanvas, rx, ry, rw, rh, 0, 0, rw, rh);
    const newBase = makeOpaqueTransparent(rw, rh);
    newBase.getContext('2d', { willReadFrequently: true }).drawImage(S.baseOriginalCanvas, rx, ry, rw, rh, 0, 0, rw, rh);
  ...
  $('resizeApply').onclick = () => {
    ...
    newSubject.getContext('2d', { willReadFrequently: true }).drawImage(S.subjectCanvas, 0, 0, nw, nh);
    const newBase = makeOpaqueTransparent(nw, nh);
    newBase.getContext('2d', { willReadFrequently: true }).drawImage(S.baseOriginalCanvas, 0, 0, nw, nh);
    S.subjectCanvas = newSubject; S.baseOriginalCanvas = newBase;
    closeCropTool();
    render();
    pushHistory();
...
function drawCropOverlay() {
  const ctx = cropOverlay.getContext('2d', { willReadFrequently: true });
...
function paintAt(x, y) {
  const w = S.subjectCanvas.width, h = S.subjectCanvas.height;
  const ctx = S.subjectCanvas.getContext('2d', { willReadFrequently: true });
...
function drawBrushCursor(x, y, r) {
  const bctx = brushOverlay.getContext('2d', { willReadFrequently: true });
